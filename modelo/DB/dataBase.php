<?php
    class DataBase
    {
        public $host=DB_HOST;
        public $user=DB_USER;
        public $pass=DB_PASS;
        public $dbname=DB_NAME;

        public $link;
        public $error;

        public function __construct()
        {
            $this->connectDB();
        }
        private function connectDB()
        {
            $this->link=new mysqli($this->host, $this->user, $this->pass, $this->dbname);
            if(!$this->link)
            {
                $this->error= "Conexion Fallida".$this->link->connect_error;
                return false;
            }
        }
        public function registerUser($query)
        {
            $sign_row=$this->link->query($query) or die ($this->link->error.__LINE__);
            if($sign_row)
            {
                /*header("Location:../../vista/public/resultado.php?msg=Los datos han sido actualizados exitosamente!!!");
                exit();*/
                /*header("Location:../../vista/public/resultado.php");
                exit();*/
            }
            else
            {
                die("Error: (".$this->link->errno.")".$this->link->error);
            }
        }
        public function select($query)
        {
            $result=$this->link->query($query) or die($this->link->error.__LINE__);
            if($result->num_rows>0)
            {
                return $result;
            }
            else
            {
                return false;
            }
        }
        public function updatePersona($query)
        {
            $update_row=$this->link->query($query) or die ($this->link->error.__LINE__);
            if($update_row)
            {
                header("Location:../vista/public/resultado.php?smg=Los datos ha sido actualizados correctamente...");
                exit();
            }
            else
            {
                die("Error:(".$this->link->errno.")".$this->link->error);
            }
        }
        public function deletePersona($query)
        {
            $delete_row=$this->link->query($query) or die($this->link->error.__LINE__);
            if($delete_row)
            {
                header("Location:../vista/public/resultado.php?smg=Los datos ha sido eliminados...");
                exit();
            }
            else
            {
                die("Error:(".$this->link->errno.")".$this->link->error);
            }
        }
    }
?>